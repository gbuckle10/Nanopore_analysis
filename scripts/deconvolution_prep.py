...
def process_chunk(chunk_df, manifest_df, uxm_atlas_bed):
    ...
    logger.info("Merging the chunk dataframe with the Illumina manifest for the meth_")
    # --- Illumina merges ---
    ...
    # Return as a dictionary containing the 4 deconvolution-ready chunks. If the chunk is empty we explicity
...
def generate_deconvolution_files(bed_file, manifest_file, output_file, uxm_atlas_file, chunk_size=1000000):
    ...
    generate_gc_illumina_deconvolution_file(methylation_df, manifest_df)
    generate_uxm_deconvolution_file(methylation_df)
    generate_aggregated_deconvolution_file("analysis/data_to_deconvolute_big.csv", uxm_atlas_file, chunk_size)
