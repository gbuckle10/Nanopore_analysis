...
def load_meth_df_from_bed(bed_dir):
    ...
    meth_df = pd.read_csv(
        bed_dir, sep='\t', header=None, usecols=[0, 1, 3, 10], names=['chr', 'start', 'mod', 'percentage']
    )
...
def generate_deconvolution_files(bed_file, manifest_file, output_file, range_atlas_file, chunk_size=1000000):
    ...
    generate_aggregated_deconvolution_file("analysis/data_to_deconvolute_big.csv", range_atlas_file, chunk_size)
