...
download_dorado_model() {
  ...
  "${DORADO_EXECUTABLE}" download --model "${DORADO_MODEL_NAME}" --models-directory "${MODEL_DIR}"
}
...
basecalling_pod5() {
  ...
  "${DORADO_EXECUTABLE}" basecaller fast,5mCG_5hmCG --modified-bases 5mCG_5hmCG --batchsize 64 data/pod5_output/all_reads.pod5 > data/basecalled_output/newcalls.bam
